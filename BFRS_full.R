...
library(Rmisc) # V.1.5.1
library(devtools)
install_github("pmartinezarbizu/pairwiseAdonis/pairwiseAdonis")
library(pairwiseAdonis) # V.0.4.1
library("ape") # V.5.7.1
...
data_otu3  # in excell, transposed DF so samples are rows
      # used heading = yes, row names = use first column, strings as factors =T, no numbers and no "X"
dim(data_otu3) # has all 6843 tax features (ASVs) and 36 samples
nb.var <- dim(data_otu3)[2] # columns (ASV variables - aka: features)
nb.var # 6843
sum(data_otu3) # 5604530, total num squ(reads)
sum_seq <-rowSums(data_otu3) # each sample has diff # reads/counts (randomly selected)
sum_seq
      
# metadata
data_grp2      # used heading = yes, row names first column & strings as factors, no numbers and no "X"
data_grp2$site <-as.factor(data_grp2$site)
data_grp2$treatment <-as.factor(data_grp2$treatment)
data_grp2$treatment <-factor(data_grp2$treatment, levels = c("control","burn","mechanical","mech-burn"))
str(data_grp2)

# tax_guild data  (combined taxonomy & guild data in excell)
data_taxo2      # used heading = yes, row names first column & strings as factors, no numbers and no "X"
str(data_taxo2) # 6843 taxa & 15 variables (tax & guild columns)

# phylogenetic tree (created in Qiime & exported)
data_PT <-ape::read.tree("tree.nwk")
data_PT

      # create phylosequ objects
OTU = otu_table(as.matrix(data_otu3), taxa_are_rows = FALSE) 
SAM = sample_data(data_grp2, errorIfNULL = TRUE) 
TAX = tax_table(as.matrix(data_taxo2)) 
PT = phy_tree(data_PT)
data_phylo <- phyloseq(OTU, TAX, SAM, PT) 
data_phylo
      # second without PT
data_phylo2 <- phyloseq(OTU, TAX, SAM)

###### normalize data with rarefaction
      # rarefy full otu
